...
@section('title', 'Forgot Password - Comginsoon')
...
<div class="w-full max-w-md" style="animation: simpleFadeIn 0.4s ease-out both;">
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
        ...
        <div class="bg-gradient-to-br from-brand-600 to-brand-700 px-8 py-8 text-center">
            ...
            <p class="text-brand-100 text-sm mt-1">We'll send you a reset link</p>
        </div>

        {{-- Form --}}
        <div class="p-8">
            @if (session('status'))
                <div class="mb-5 p-3 rounded-lg bg-brand-50 border border-brand-200 text-sm text-brand-700">
                    {{ session('status') }}
                </div>
            @endif
